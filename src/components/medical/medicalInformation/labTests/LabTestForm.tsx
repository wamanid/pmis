...
interface LabTestFormProps {
  ...
  onSubmit: (labTest: LabTest) => void;
  onCancel: () => void;
  mode: 'create' | 'edit' | 'view';
}

const LabTestForm: React.FC<LabTestFormProps> = ({ labTest, onSubmit, onCancel, mode }) => {
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      setFormData((prev) => ({ ...prev, result_document: file.name }));
          ...
          {!isReadOnly && (
            <div className="flex items-center justify-end gap-3 pt-4 border-t">
              ...
              >
                ...
                {loading ? 'Saving...' : mode === 'create' ? 'Create Test' : 'Update Test'}
              </Button>
            </div>
          )}

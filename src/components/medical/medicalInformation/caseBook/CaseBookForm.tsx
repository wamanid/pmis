...
import {BmiRecord, Case, getBmiRecord} from "../../../../services/medical/medicalInformation/medical";
...
const CaseBookForm: React.FC<CaseBookFormProps> = ({
  setBloodGroups, bloodGroups, prisoners, setPrisoners, loader, setLoader, checkupTypes, setNewDialogLoader, presentations,
  caseBook,
  onSubmit,
  onCancel,
  mode,
  ...
  useEffect(() => {
    if (caseBook) {
      fetchPatientBmi(caseBook.prisoner)
    }
  }, [caseBook]);

  async function fetchPatientBmi(id) {
    setNewDialogLoader(true)
    try {
      await getBmi(setBmiRecords, id)
  ...
  const handleInputChange = async (field: keyof CaseBook, value: any) => {
    setFormData((prev) => ({ ...prev, [field]: value }));

    if (field === 'prisoner') {
      setNewDialogLoader(true)
      try {
        await getBmi(setBmiRecords, value)
      }catch (error) {
        handleCatchError(error)
      }
      finally {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!formData.grade.trim()) {
      toast.error('Please enter the grade');
      return;
    }

    if (!formData.referral.trim()) {
      toast.error('Please enter the referral');
      return;
    }
    ...
    if (!formData.notes.trim()) {
     ...
     if (!formData.bmi.trim()) {
      toast.error('Please enter the BMI');
      return;
    }

     if (!formData.history.trim()) {
      toast.error('Please enter the medical history');
      return;
    }

     if (!formData.presentation_of_patient.trim()) {
      toast.error('Please enter the presentation of the patient');
      return;
    }
             ...
             <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="bmi">BMI Record <span className="text-red-500">*</span></Label>
                    <Select
                      value={formData.bmi}
                    ...
                    >
                      <SelectTrigger id="bmi">
                        <SelectValue placeholder="Select BMI record" />
                      </SelectTrigger>
                      <SelectContent>
                        {bmiRecords.map((bmi) => (
                          <SelectItem key={bmi.id} value={bmi.id}>
                            {bmi.bmi} - {bmi.classification_name} - {changeDate(bmi.created_datetime)}
                          </SelectItem>
                        ))}
                      </SelectContent>
                    </Select>
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="presentation_of_patient">
                      Presentation of Patient <span className="text-red-500">*</span>
                    </Label>
                    <Select
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="history">Medical History <span className="text-red-500">*</span></Label>
                    <Textarea
                      id="history"
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="referral">Referral <span className="text-red-500">*</span></Label>
                    <Textarea
                      id="referral"

...
import MedicalDetails from "../medical/medicalInformation/MedicalDetails";
import PrisonerPropertyScreen from "../property/PrisonerPropertyScreen";
import HousingAllocationScreen from "../station/HousingAllocationScreen";
import { 
  Prisoner, 
  PrisonerBiodata,
  NextOfKin,
...
const PrisonerAdmissionScreen: React.FC = () => {
  ...
  // Search for existing prisoner - handled by PrisonerSearchScreenWider component
  const handleSearch = () => {
    // Search is now handled by the PrisonerSearchScreenWider component
    // which fetches from the API directly
  };
  ...
  const handleBiometricScan = async () => {
    ...
    setTimeout(() => {
      ...
      // 1. Get fingerprint data from hardware scanner
      // 2. Send to API for matching
      // 3. Return matched prisoner records
      setIsScanning(false);
      toast.info("Biometric search would be implemented with actual hardware scanner and API integration");
    }, 3000);
  };
  ...
  const generatePrisonerNumbers = async () => {
    try {
      ...
      console.log("✅ Prisoner number generated:", response);
      ...
      console.log("💾 Reservation stored to localStorage:", {
        key: "pmis_prisoner_number_reservation",
        value: response,
        stored: localStorage.getItem("pmis_prisoner_number_reservation")
      });
        ...
        <CardContent className="pt-6">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <div className="flex-1 min-w-[20px] h-0.5 bg-gray-200">
              <div className={`h-full transition-all ${currentStep >= 2 ? "bg-[#650000]" : ""}`} style={{ width: currentStep >= 2 ? "100%" : "0%" }} />
            </div>

            {/* Step 2: Biodata */}
            <div className="flex items-center gap-2">
              <div className={`flex items-center justify-center w-8 h-8 rounded-full ${currentStep >= 2 ? "bg-[#650000] text-white" : "bg-gray-200"}`}>
                2
              </div>
              ...
              <div className="flex-1 min-w-[20px] h-0.5 bg-gray-200">
                <div className={`h-full transition-all ${currentStep >= 3 ? "bg-[#650000]" : ""}`} style={{ width: currentStep >= 3 ? "100%" : "0%" }} />
              </div>

              {/* Step 3: Medical */}
              <div className="flex items-center gap-2">
                <div className={`flex items-center justify-center w-8 h-8 rounded-full ${currentStep >= 3 ? "bg-[#650000] text-white" : "bg-gray-200"}`}>
                  3
                </div>
              ...
              <div className="flex-1 min-w-[20px] h-0.5 bg-gray-200">
                <div className={`h-full transition-all ${currentStep >= 4 ? "bg-[#650000]" : ""}`} style={{ width: currentStep >= 4 ? "100%" : "0%" }} />
              </div>

              {/* Step 4: Property */}
              <div className="flex items-center gap-2">
                <div className={`flex items-center justify-center w-8 h-8 rounded-full ${currentStep >= 4 ? "bg-[#650000] text-white" : "bg-gray-200"}`}>
                  4
                </div>
                <span className={currentStep >= 4 ? "text-[#650000]" : "text-gray-500"}>
                  Property
                </span>
              </div>

              <div className="flex-1 min-w-[20px] h-0.5 bg-gray-200">
                <div className={`h-full transition-all ${currentStep >= 5 ? "bg-[#650000]" : ""}`} style={{ width: currentStep >= 5 ? "100%" : "0%" }} />
              </div>
      {/* Step 1: Admission Setup */}
      {currentStep === 1 && (
        <Card>

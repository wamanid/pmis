...
export function DataTable({
  ...
  onSort,
  externalSearch,
  // NEW: optional renderExpandedRow to show a full-width sub-row under a row
  renderExpandedRow,
}: DataTableProps & {
  data?: any[];
  loading?: boolean;
          ...
          <div className="text-sm text-muted-foreground">
            {(() => {
              ...
              const effectivePageSize =
                pageSize === -1 ? totalRecords : pageSize;
              const totalPages = Math.max(
                1,
                Math.ceil(totalRecords / Math.max(1, effectivePageSize))
              );
              const startRecord =
                totalRecords === 0
                  ? 0
              ...
              const endRecord = Math.min(
                totalRecords,
                startRecord + (rowsToRender.length || 0) - 1
              );

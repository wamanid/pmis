...
export function ManualLockupTableView({ lockups }: ManualLockupTableViewProps) {
  ...
  const groupLockups = (): GroupedLockup[] => {
    ...
    filtered.forEach(lockup => {
      ...
      if (lockup.prisoner_category === categoryIds.convict) {
        grouped[key].convictTotal += lockup.count;
      } else if (lockup.prisoner_category === categoryIds.remand) {
        grouped[key].remandTotal += lockup.count;
      } else if (lockup.prisoner_category === categoryIds.debtor) {
        grouped[key].debtorTotal += lockup.count;
      } else if (lockup.prisoner_category === categoryIds.lodger) {
        grouped[key].lodgerTotal += lockup.count;
      }

      grouped[key].grandTotal += lockup.count;
    });
  ...
  const buildDetailGrid = (records: ManualLockup[]): DetailGrid => {
    ...
    records.forEach(record => {
      ...
      if (record.prisoner_category === categoryIds.convict) category = 'convict';
      else if (record.prisoner_category === categoryIds.remand) category = 'remand';
      else if (record.prisoner_category === categoryIds.debtor) category = 'debtor';
      else if (record.prisoner_category === categoryIds.lodger) category = 'lodger';
                            ...
                            <table className="w-full border-collapse border">
                              ...
                              <tbody>
                                ...
                                <tr className="bg-primary/10">
                                  ...
                                  <td className="p-2 text-center">{group.grandTotal}</td>
                                </tr>
                              </tbody>
                            </table>
                          </div>
